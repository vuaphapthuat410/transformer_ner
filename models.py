...
class BertGRUCRF(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        self.bigru = nn.GRU(config.hidden_size, (config.hidden_size) // 2, dropout=config.dropout, batch_first=True,
                              bidirectional=True) # remove dropout because only one layer
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.crf = CRF(num_tags=config.num_labels, batch_first=True)
        self.init_weights()
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        sequence_output = self.dropout(sequence_output)
        gru_output, hc = self.bigru(sequence_output)
